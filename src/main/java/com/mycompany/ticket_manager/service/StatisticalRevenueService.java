 ...
 * @author chien
 */
public class StatisticalRevenueService {

    StatisticalRepository statisticalRepository;

    public StatisticalRevenueService() {
    ...
    public Response<List<Map<String, Object>>> revenue(int style, String day, String month, String year) {
        try {
            ...
            ResultSet resultSet = this.statisticalRepository.revenue(timeStart, timeEnd);
    ...
    private List<Map<String, Object>> followDay(ResultSet resultSet) throws SQLException {
        ...
        while (resultSet.next()) {
            ...
            water += resultSet.getInt("priceWater");
            popcorn += resultSet.getInt("pricePopcorn");
        }

        sum = ticket + water + popcorn;
        Map<String, Object> row = new HashMap<>();
        row.put("date", date);
        row.put("ticket", ticket);
        row.put("water", water);
        row.put("popcorn", popcorn);
        row.put("sum", sum);
        listData.add(row);
        return listData;
    }
